...
export default function PortalPage() {
  ...
  async function loadReservations() {
    ...
    if (error) {
      setMessage(`No fue posible leer reservas: ${error.message}`);
      return;
    }
  ...
  async function loadLogs() {
    ...
    if (error) {
      setMessage(`No fue posible leer historial: ${error.message}`);
      return;
    }
  ...
  const todayReservations = useMemo(
    () => nextReservations.filter((r) => r.date === todayISO()),
    [nextReservations]
  );
  ...
  async function addLog(reservationId, action, detail) {
    if (!supabase) return;
    const { error } = await supabase.from("reservation_logs").insert({
  ...
  async function handleSubmit(event) {
    ...
    if (conflict) {
      setMessage(`Horario no disponible. Ya existe reserva de ${conflict.start} a ${conflict.end} para ${conflict.email || conflict.user}.`);
      return;
    }
  ...
  function editReservation(reservation) {
    ...
    setForm({ date: reservation.date, start: reservation.start, end: reservation.end, notes: reservation.notes || "" });
    setActive("agenda");
  ...
  async function cancelReservation(reservation) {
    const { error } = await supabase
      .from("reservations")
      .update({ estado: "cancelada", cancelled_at: new Date().toISOString(), cancelled_by_email: userEmail, updated_at: new Date().toISOString() })
  ...
  if (loading) {
    return <main className="portalMain"><section className="portalCard emptyState"><h1>Cargando ERP MC&amp;DJ...</h1><p>{message}</p></section></main>;
  }
      ...
      <main className="portalShell">
        <section className="portalMain">
          <form className="portalCard reservationForm" onSubmit={handleLogin} style={{ maxWidth: 520, margin: "48px auto" }}>
            <div className="sectionTitle"><h1>MC&amp;DJ ERP</h1><p>Inicia sesion para usar la Agenda NOMIPAQ.</p></div>
  ...
  return (
    <div className="portalShell">
      <aside className="portalSidebar">
        <a className="portalBrand" href="/portal"><strong>MC&amp;DJ ERP</strong><span>Sistema interno</span></a>
      ...
      <main className="portalMain">
        <header className="portalTopbar portalTopbarBlue">
          <div><p>ERP MC&amp;DJ</p><h1>{active === "agenda" ? "Agenda NOMIPAQ" : "Dashboard inicial"}</h1></div>
          <div className="roleBox"><span>{userEmail}</span><strong>{roleLabel}</strong><button type="button" onClick={handleLogout}>Cerrar sesion</button></div>
        </header>
          ...
          <section className="agendaLayout">
            <form className="portalCard reservationForm" onSubmit={handleSubmit}>
              ...
              <div className="timeGrid">
                <Field label="Inicio"><input type="time" value={form.start} onChange={(e) => setForm({ ...form, start: e.target.value })} /></Field>
                <Field label="Fin"><input type="time" value={form.end} onChange={(e) => setForm({ ...form, end: e.target.value })} /></Field>
              </div>
              ...
              <div className="formActions"><button className="btn btn-primary" type="submit">{editingId ? "Guardar cambio" : "Reservar"}</button>{editingId ? <button className="btn btn-secondary" type="button" onClick={resetForm}>Cancelar edicion</button> : null}</div>
            </form>

            <div className="portalCard tableCard">
              <div className="sectionTitle"><h2>Reservas activas</h2><p>Una sola licencia disponible. No se permiten empalmes.</p></div>
              <div className="reservationList">
                {nextReservations.length ? nextReservations.map((reservation) => (
                  <article className="reservationItem" key={reservation.id}>
                    <div><strong>{reservation.date} - {reservation.start} a {reservation.end}</strong><span>{reservation.user} - {reservation.email}</span>{reservation.notes ? <small>{reservation.notes}</small> : null}</div>
                    <div className="reservationActions"><button type="button" onClick={() => editReservation(reservation)}>Reagendar</button><button type="button" onClick={() => cancelReservation(reservation)}>Cancelar</button></div>
                  </article>
            ...
            <div className="portalCard logCard">
              <div className="sectionTitle"><h2>Historial</h2><p>Bitacora operativa para auditoria interna.</p></div>
              <div className="logList">{logs.length ? logs.map((log) => <div key={log.id}><strong>{log.action}</strong><span>{log.date} - {log.detail}</span></div>) : <p>No hay historial todavia.</p>}</div>
            </div>
          </section>
        ) : null}
